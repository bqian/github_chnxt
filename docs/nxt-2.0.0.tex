...
\end{lstlisting}
rotates {\tt NXT\_JOINT1} continuously until a stop function is called. The stop function here we used is 
{\tt stopOneJoint()}, which can only stop the specified joint.Other two stop functions are {\tt stopTwoJoints()} 
and {\tt stopAllJoints()}, which stops two joints and all joints respectively. The next four lines,
\end{comment}
% end comment
\begin{lstlisting}
...
After the {\bf while} loop, the NXT vehicle RTC program ends with the four lines shown below.
\begin{lstlisting}
/* Disconnect from NXT */
nxt.disconnect();
\end{lstlisting}
