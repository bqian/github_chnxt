...
%\subsection{The speed and joint setting slide bars\label{sec:gui_speedjoint}}
...
set zero positions NXT joints. In this section, we will present a demo program, 
{\tt setSpeedRatios.ch}, to illustrate how to set speed ratios for the joints 
of NXT.
\end{comment}
...
\end{lstlisting}
rotates {\tt NXT\_JOINTA} continuously until a stop function is called. The stop 
function here we used is {\tt stopOneJoint()}, which can only stop the specified 
joint. Other two stop functions are {\tt stopTwoJoints()} and {\tt stopAllJoints()}, 
which stops two joints and all joints respectively. The next four lines,
\end{comment}
% end comment
\begin{lstlisting}
...
%    nxt.setJointSpeed(NXT_JOINTA, 50);
%
